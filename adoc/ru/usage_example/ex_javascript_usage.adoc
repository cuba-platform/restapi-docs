...
Чтобы отобразить список покупателей, на сервер отправляется <<rest_api_v2_ex_get_entities_list,запрос на получение списка экземпляров>> сущности `sales_Customer`, передавая значение `oauthToken` в заголовке `Authorization`.
